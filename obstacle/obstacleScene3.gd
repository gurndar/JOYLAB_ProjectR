...
export var obstacle3scene: PackedScene

var spawn = 0
var obstacle_instance = null

func _on_perception_system_spawn_3():
	if obstacle3scene:
		obstacle_instance = obstacle3scene.instance()
		obstacle_instance.position = Vector2(720, -100) # y=100으로 테스트
